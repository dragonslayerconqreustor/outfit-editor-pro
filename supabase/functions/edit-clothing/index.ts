...
serve(async (req) => {
  ...
  try {
        ...
        messages: [
          {
            ...
            content: [
              {
                type: 'text',
                text: `Change the clothing in this image to: ${prompt}. Keep the person, pose, background, and everything else exactly the same. Only modify the clothing. Make it look natural and realistic.`
              },
              {
                type: 'image_url',
                image_url: {
    ...
    if (!response.ok) {
      ...
      throw new Error('Failed to edit clothing');
    }
